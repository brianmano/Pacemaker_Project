...
#        x=0
    ...
    def init_window(self):
        ...
        def animate(i):
           self.line.set_ydata(self.amp * np.sin(self.x+i/10.0))  # update the data
           return self.line,
...
#        self.buttonPlot = Button(self,text="Plot",command=self.Plot,width=12)
        self.buttonPlot = Button(self,text="Plot",width=12, command=lambda:self.set_amp(self.textAmplitude.get()))
        ...
        self.buttonToggle = Button(self,text="toggle",command=self.Clear,width=12)
        self.buttonToggle.grid(row=2,column=3)
